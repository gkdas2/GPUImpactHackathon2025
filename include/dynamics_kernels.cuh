...
__global__ void update(int num_elements, T dt,
  ...
  // Start element tag
  int start_elem_tag = blockIdx.x * elements_per_block;
  if (start_elem_tag >= num_elements) return;

  // Final element tag
  int end_elem_tag = start_elem_tag + elements_per_block - 1;

  __shared__ T
      element_mass_matrix_diagonals[dof_per_element * elements_per_block];
  ...
  __shared__ T element_strain[nodes_per_element * 6 * elements_per_block];
  __shared__ T element_stress[nodes_per_element * 6 * elements_per_block];
  __shared__ int this_element_nodes[nodes_per_element * elements_per_block];

  // Define the local thread id
  int tid = threadIdx.x;
  // printf("%i \n", tid);

  // For each thread id less than 12*5=60, initialize the shared arrays
  ...
  if (tid < nodes_per_element * 6 * elements_per_block) {
  ...
  if (tid < nodes_per_element * elements_per_block) {
    this_element_nodes[tid] =
        d_element_nodes[nodes_per_element * start_elem_tag + tid];
  }
  __syncthreads();
  ...
  Analysis::template get_element_dof<
      ...
      elements_per_block>(tid, this_element_nodes, d_global_xloc, element_xloc);

  // Get the element degrees of freedom
  Analysis::template get_element_dof<
      Analysis::spatial_dim, Analysis::dof_per_element, Analysis::dof_per_node,
      elements_per_block>(tid, this_element_nodes, d_global_dof, element_dof);

  __syncthreads();
  // Calculate element mass matrix

  Analysis::element_mass_matrix_gpu<elements_per_block>(
      tid, d_material->rho, element_xloc, element_dof,
      element_mass_matrix_diagonals, nodes_per_elem_num_quad);
  ...
  Analysis::template get_element_dof<
      Analysis::spatial_dim, Analysis::dof_per_element, Analysis::dof_per_node,
      elements_per_block>(tid, this_element_nodes, d_global_mass,
  ...
  Analysis::calculate_f_internal_gpu<elements_per_block>(
      tid, element_xloc, element_dof, element_internal_forces, d_material);
  __syncthreads();
