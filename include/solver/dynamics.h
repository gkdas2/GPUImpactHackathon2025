 ...
 //  objects
  ...
  Dynamics(Mesh<T, nodes_per_element> *input_mesh, Material *input_material,
      ...
      : mesh(input_mesh),
        ...
        inelastic_energy(new T[mesh->num_elements * num_quadrature_pts]),
  ...
  void solve(double dt, double time_end, int export_interval)
  {
    ...
    update<T, spatial_dim, nodes_per_element>(mesh->num_nodes, mesh->num_elements, ndof, dt, material, wall, mesh,
    element_nodes, vel, global_xloc, global_dof, global_acc, global_mass,
         ...
         internalEnergy, inelastic_energy, temperature, density, time);
